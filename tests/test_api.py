...
def test_validate_config_valid():
    response = client.post(
        "/validate/config", json={"file_path": "samples/sample_config.yaml"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_validate_db_valid():
    ...
    try:
        ...
        conn.close()
        response = client.post(
            "/validate/db", json={"db_url": db_url, "table": "users", "condition": None}
        )
        assert response.status_code == 200
        assert response.json()["valid"] is True
    finally:
...
def test_validate_db_condition_fails():
    ...
    try:
        ...
        response = client.post(
            "/validate/db",
            json={"db_url": db_url, "table": "users", "condition": "age < 18"},
        )
        assert response.status_code == 400
